...
def feed_stock_data_list(dummy_list = list):
...
def read_monthly_csv(month_number = str,year_number = str,company_name = str):
    ...
    for d in range(1,32):
        try:
            input_date = date(year_number,month_number,d)
            input_date = nse_downloader.verify_date(input_date)
            if (input_date == None):
              continue
            else:
                daily_list = read_daily_csv(input_date,company_name)
                if ( daily_list != None):
